...
def gen_pairs():
    ...
    eval_pairs = pool
    out_f = './user_study_targetSH_pairs'
    ...
    for pair_id in tqdm.tqdm(sample_pairs.values()):
        ...
        for _, m in enumerate(candi.keys()):
            ...
            n_frames_dir = glob.glob(f"{m_path}/{pair[0]}/{pair[1]}/{candi[m]['post_misc']}/n_frames={n_frames}")
            # print(f"{m_path}/{pair[0]}/{pair[1]}/{candi[m]['post_misc']}/n_frames=*")
            # print(n_frames_dir)
            # exit()
