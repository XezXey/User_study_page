...
def gen_pairs():
    ...
    for pair_id in tqdm.tqdm(sample_pairs.values()):
        ...
        for m in candi.keys():
            ...
            # Relighting vids
            if do_n1:
                frames = glob.glob(f"{m_path}/{pair[0]}/{pair[1]}/{candi[m]['post_misc']}/n_frames={n_frames}/res_frame*.png")
                ...
                res_vid = f"{m_path}/{pair[0]}/{pair[1]}/{candi[m]['post_misc']}/n_frames={n_frames}/res_rt_nof.mp4"
